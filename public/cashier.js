...
app.formatCashBundle = function(bundleData, header){
  ...
  output += "<ul>"
  _(quantityNames).forEach(function(name,denom){
    var q = quantityFormatter.format(bundleData['quantities'][denom])
...
app.displayDrawer = function(){
  $.ajax({url:"/drawer"})
   .done(function(drawerData){
    var drawerHTML = app.formatCashBundle(drawerData, "The cash register contains")
...
$( document ).ready(function() {
  ...
  $("#go").click(function(e){
    ...
    var dataToPost = {"payment":payment,"price":price}
    $.ajax({url:"/transaction", method:"POST", data: dataToPost})
